...
These examples use real plugin APIs and service lookups rather than mock bridge abstractions. They also show the current AsyncResult/SyncResult chaining model, where async chains use `then(...)` and sync handoffs use `thenSync(...)`. The full Spigot example keeps optional plugin integrations in separate registrar classes so the main plugin entrypoint can load without Essentials, LuckPerms, or PlaceholderAPI present. If you want to consume providers from another plugin, see the API developer guide in [API_DEVELOPERS.md](../../API_DEVELOPERS.md).
